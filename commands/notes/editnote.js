...
module.exports = {
    ...
    aliases: ['edit'],
    cooldown: 5,
    usage: '< owner mention > < name >',
    async execute(client, message, args, db) {
        if (!args[0]) return message.channel.send('Please mention the main owner of the note!')
        ...
        if (await db.get(`${owner.id}_${args[1].toLowerCase()}`).coowner !== message.author.id && await db.get(`${owner.id}_${args[1].toLowerCase()}`).owner !== message.author.id) return message.channel.send('You aren\'t a co-owner or owner of that note!')
